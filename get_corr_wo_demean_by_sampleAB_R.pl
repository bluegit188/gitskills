...
my $colX=$ARGV[4];
 ...
 my $cmdx="
cat $filename  |mySampleTag.pl 1 $colDate >/tmp/$filename.samAB
";
system("$cmdx");
...
my $cmd1 = <<EOF;
regdata<-fread(file="/tmp/$filename.samAB",header=T)
#summary(regdata)

syms=sort(unique(regdata\$sampleTag))
...
for ( i in syms)
{
  ...
  regdataNow<- regdata[regdata\$sampleTag == i,]
...
names(allResults)=c("sampleTag", "cors","betas","signR2s", "nobs")
allResults
