...
class Set:
    ...
    def __init__(self, config, df=None):
        self.trade_num = int(config.trade_num)
        self.config = config
        ...
        self.tcost = float(config.tcost)
        self.minute = float(config.minute)
        if df is not None:
            self.df = df
        else:
...
class Col(Set):
	...
	# terminal visualization
        ret_msg = ['General:']
        header = ['Period', 'Fcst','FcstMax','FcstMin', 'Y_value', 'X_Value', 'Constraint', 'Weight']
        l = []
        for i in range(self.trade_num):
            l.append(['P%g' % i])
        ...
        ret_msg.append(' ')
        ret_msg.append(' ')
        ret_msg.append('ContractLeveL:')
        header =  ['Period', 'GrossMean', 'GrossSTD', 'GrossSharpe', 'NetMean', 'NetSTD', 'NetSharpe']
        l = []
        for i in range(self.trade_num):
            l.append(['P%g' % i])
        ...
        ret_msg.append(' ')
        ret_msg.append(' ')
        ret_msg.append('PortfolioLevel:')
        header = ['Period', 'GrossMean', 'GrossSTD', 'GrossSharpe',
                  'NetMean', 'NetSTD', 'NetSharpe', 'DivNum','NetMDD']
        k = []
        for i in range(self.trade_num):
...
class Port:
    def __init__(self, col):
        ...
        self.p_net_sharpe = get_sharpe(portfolio_net.net_return)        # port pnl
        self.p_gross_sharpe = get_sharpe(portfolio_gross.gross_return)
        self.c_net_sharpe = get_sharpe(result.net_return)               # perCom Pnl 
	...
	# terminal visualization
        ...
        l[0].append(self.c_gross_sharpe)
        l[0].append(self.c_net_sharpe)
        l[0].append(self.p_gross_sharpe)
        l[0].append(self.p_net_sharpe)
    ...
    ## this is for the summary stats  file: overview.csv
    def ovr(self):
        k = pd.DataFrame()
        for i in range(self.col.trade_num):
            k['Forecast_%g' % (i+1)] = [self.col.fcst_list[i]]
        for i in range(self.col.trade_num):
            k['Y_value_%g' % (i+1)] = self.col.y_list[i]
        ...
        for i in range(self.col.trade_num):
            k['X_value_%g' % (i+1)] = self.col.x_list[i]
        if self.col.trade_num > 1:
            for i in range(self.col.trade_num):
                k['Weight_%g' % (i+1)] = self.col.w_list[i]
        #for i in range(self.col.trade_num):
        ...
        k['plNetStd'] = self.plNetStd
        k['pplNetStd'] = self.pplNetStd
        return k
