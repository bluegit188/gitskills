...
($#ARGV+2) ==5 || die 
"Usage:get_corr_wo_demean_R.pl file.txt(header) colDate colY colX
...
for ( i in syms)
{
  ...
  # without demean
  cor=round(corrwodm(regdata\$$yName, regdata\$$xName),digits=7)
...
names(allResults)=c("ASSET4", "cors","signR2s", "nobs")
allResults
