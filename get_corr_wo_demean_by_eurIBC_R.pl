...
my $colX=$ARGV[4];
 ...
 my $cmdx="
cat $filename |myIsEurIBC.pl  1 $colSym >/tmp/$filename.eurIBC
";
system("$cmdx");
...
my $cmd1 = <<EOF;
regdata<-fread(file="/tmp/$filename.eurIBC",header=T)
#summary(regdata)

syms=sort(unique(regdata\$isEurIBC))
...
for ( i in syms)
{
  ...
  regdataNow<- regdata[regdata\$isEurIBC == i,]
...
names(allResults)=c("isEurIBC", "cors","betas","signR2s", "nobs")
allResults
