...
($#ARGV+2)==2 || die
"Usage: portara_plot_fvoof.pl SYM
        Plot FVOO_fastEma of a given future\n";
...
my $sym=$ARGV[0]; # portara text file
    ...
    my $cmd="
portara_get_FVOOF.pl $sym |mygetcols.pl 2 4 |fgrep -v DATE > /tmp/c.txt
datesXmgr.pl /tmp/c.txt 1 |mygetcols.pl 1 3 2 > /tmp/$sym.txt.fvoo
echo \"title \\\"FVOOF: $sym\\\"\" > /tmp/title.txt
xmgrByDate  -batch /tmp/title.txt /tmp/$sym.txt.fvoo&
";
