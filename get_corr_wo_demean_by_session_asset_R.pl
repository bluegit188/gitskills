...
($#ARGV+2) ==6 || die 
"Usage:get_cor_by_sym.pl file.txt(header) colSym colDate colY colX
...
my $colX=$ARGV[4];
 ...
 my $cmdx="
cat $filename |myAssetSession_four_aseets.pl  1 $colSym |gawk '{print \$0,\$(NF-1)\"_\"\$NF}' >/tmp/$filename.sessionAsset4
";
system("$cmdx");
...
my $cmd1 = <<EOF;
regdata<-fread(file="/tmp/$filename.sessionAsset4",header=T)
#summary(regdata)

syms=sort(unique(regdata\$SESSION_ASSET))
...
for ( i in syms)
{
  ...
  regdataNow<- regdata[regdata\$SESSION_ASSET == i,]
...
names(allResults)=c("SESSION_ASSET", "cors","betas","signR2s", "nobs")
allResults
